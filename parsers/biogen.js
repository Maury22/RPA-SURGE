// ============================================================
// parsers/biogen.js — Parser específico para Biogen Argentina SRL
// ============================================================

const { limpiarImporte, normalizarTexto, textoEnLinea, REGEX_GTIN } = require('./utils');

const CUIT_BIOGEN = '30709724912';

function detectar(textoPlano) {
    if (textoPlano.includes(CUIT_BIOGEN) || textoPlano.includes('30-70972491-2')) return true;
    if (/BIOGEN ARGENTINA/i.test(textoPlano)) return true;
    return false;
}
...
function extraerDatos(textoOCR) {
    ...
    const cuit = CUIT_BIOGEN;

    let puntoVenta = '', numeroComprobante = '';
    const matchNro = plano.match(/(?<![0-9])([0-9]{4,5})[\s\-_]+([0-9]{8})(?![0-9])/);
    if (matchNro) {
...
module.exports = { CUIT: CUIT_BIOGEN, nombre: 'Biogen', detectar, extraerDatos, extraerDatosAnexo };