// ============================================================
// parsers/monteverde.js — Parser específico para Monte Verde S.A.
// ============================================================

const { limpiarImporte, normalizarTexto, textoEnLinea, REGEX_GTIN } = require('./utils');

const CUIT_MONTEVERDE = '30700743140';

function detectar(textoPlano) {
    if (textoPlano.includes(CUIT_MONTEVERDE) || textoPlano.includes('30-70074314-0')) return true;
    if (/Monte Verde S\.?A\.?/i.test(textoPlano)) return true;
    return false;
}
...
function extraerDatos(textoOCR) {
    ...
    const cuit = CUIT_MONTEVERDE;

    let puntoVenta = '', numeroComprobante = '';
    const matchNro = plano.match(/Factura\s*([0-9]{4,5})\s*[-–—]\s*([0-9]{8})/i) || 
    ...
    let fecha = '';
    const matchFecha = plano.match(/(?:Munro,?\s*)?([0-9]{2})[\s\/\-.]+([0-9]{2})[\s\/\-.]+([0-9]{4})/i) || 
...
module.exports = { CUIT: CUIT_MONTEVERDE, nombre: 'Monte Verde', detectar, extraerDatos, extraerDatosAnexo };