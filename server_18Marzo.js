...
module.exports = function iniciarServidorBackend(rutaSeguraDatos, rutaCodigo, rutaEscritorio) {
    ...
    function convertirPdfAImagenes(pdfPath, outputPrefix = 'pagina') {
        ...
        const rutaProduccion = path.join(path.dirname(process.execPath), 'poppler', 'Library', 'bin', 'pdftoppm.exe');
    ...
    function extraerTextoCapaDelPDF(pdfPath) {
        try {
            ...
            const rutaProduccion = path.join(path.dirname(process.execPath), 'poppler', 'Library', 'bin', 'pdftotext.exe');
